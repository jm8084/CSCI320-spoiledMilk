        ...
        try:
            if ((res == 'm') or (res == 'made')):
                ...
                if action == 'a' or action == 'A' or action == 'accept' or action == 'Accept':
                    ...
                    date = input("Change the due date of the tool, leave blank to not change YYYY-MM-DD: ")
                    try:
                        selected_request=requests[int(index)]
                        if(date==""):
                            date=selected_request[4]
                        ...
                        date_result = cur.execute(
                             f"UPDATE request SET datereturned = '{date}' WHERE barcode = {selected_request[0]} AND username = '{selected_request[2]}' AND daterequired = '{selected_request[3]}' AND status = 1")
